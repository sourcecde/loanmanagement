...
function getBorrowerLoanCount($borrower_id)
{
  $query = sprintf("SELECT count(1) num FROM tbl_loans  WHERE borrower_id='%s'", $borrower_id);
  $result = mysql_query($query);
  $row = mysql_fetch_array($result);
  $num=$row['num'];
  return $num;
...
function getLenderLoanCount($lender_id)
{
  $query = sprintf("SELECT count(1) num FROM tbl_loans  WHERE lender_id='%s'", $lender_id);
  $result = mysql_query($query);
  $row = mysql_fetch_array($result);
  $num=$row['num'];
  return $num;
...
function getBorrowerLoans($borrower_id)
{
    $result = mysql_query(sprintf("SELECT loan_id,l.borrower_id 'borrower_id',b.name 'borrower_name',l.lender_id 'lender_id', c.name 'lender_name',amount,start_date FROM `tbl_loans` l left join `tbl_users` b on l.borrower_id=b.user_id left join `tbl_users` c on l.lender_id=c.user_id WHERE l.borrower_id='%s'", $borrower_id));
  $values = array();
  $i=0;
  while($row = mysql_fetch_array($result)) 
  {
...
function getLenderLoans($lender_id)
{
    $result = mysql_query(sprintf("SELECT loan_id,l.borrower_id 'borrower_id',b.name 'borrower_name',l.lender_id 'lender_id', c.name 'lender_name',amount,start_date FROM `tbl_loans` l left join `tbl_users` b on l.borrower_id=b.user_id left join `tbl_users` c on l.lender_id=c.user_id WHERE l.lender_id='%s'", $lender_id));
  $values = array();
  $i=0;
  while($row = mysql_fetch_array($result)) 
  {
...
function getBorrowerLoanDetails($borrower_id,$loan_id){
  $query = sprintf("SELECT loan_id,l.borrower_id 'borrower_id',b.name 'borrower_name',b.address 'borrower_address',b.phone 'borrower_phone',b.email 'borrower_email',l.lender_id 'lender_id', c.name 'lender_name',c.address 'lender_address',c.phone 'lender_phone',c.email 'lender_email',amount,start_date,interest_rate,term,term_freq,schedule,notes FROM `tbl_loans` l left join `tbl_users` b on l.borrower_id=b.user_id left join `tbl_users` c on l.lender_id=c.user_id WHERE l.borrower_id='%s' and loan_id='%s'",$borrower_id, $loan_id);
  $result = mysql_query($query);
  $row = mysql_fetch_array($result);
  return $row;
}

function getLenderLoanDetails($lender_id,$loan_id){
  $query = sprintf("SELECT loan_id,l.borrower_id 'borrower_id',b.name 'borrower_name',b.address 'borrower_address',b.phone 'borrower_phone',b.email 'borrower_email',l.lender_id 'lender_id', c.name 'lender_name',c.address 'lender_address',c.phone 'lender_phone',c.email 'lender_email',amount,start_date,interest_rate,term,term_freq,schedule,notes FROM `tbl_loans` l left join `tbl_users` b on l.borrower_id=b.user_id left join `tbl_users` c on l.lender_id=c.user_id WHERE l.lender_id='%s' and loan_id='%s'",$lender_id, $loan_id);
  $result = mysql_query($query);
  $row = mysql_fetch_array($result);
  return $row;
}
